...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    setMyDate(today); setAdDate(today);
  },[]);
  ...
  async function create(){
    ...
    try{ await api('/api/reservations', { method:'POST', body: JSON.stringify(payload) });
        ...
        <div className="container">
          ...
          {session && (
            <div id="app">
                ...
                <div id="view-book">
                  ...
                  <div className="row">
                    ...
                    <div className="col"><label>بداية الاجتماع</label>
                      <div className="row">
                        <div className="col"><select value={form.sh} onChange={e=>setForm({...form, sh:e.target.value})}>{hours.map(h=>(<option key={h} value={h}>{h}</option>))}</select></div>
                        <div className="col"><select value={form.sm} onChange={e=>setForm({...form, sm:e.target.value})}>{mins.map(m=>(<option key={m} value={m}>{m}</option>))}</select></div>
                      </div>
                    </div>
                    <div className="col"><label>نهاية الاجتماع</label>
                      <div className="row">
                        <div className="col"><select value={form.eh} onChange={e=>setForm({...form, eh:e.target.value})}>{hours.map(h=>(<option key={h} value={h}>{h}</option>))}</select></div>
                        <div className="col"><select value={form.em} onChange={e=>setForm({...form, em:e.target.value})}>{mins.map(m=>(<option key={m} value={m}>{m}</option>))}</select></div>
                      </div>
                    </div>
                  </div>
              ...
              {tab==='my' && (
                <div id="view-my">
                  <div className="row"><div className="col"><label>التاريخ</label><input type="date" value={myDate} onChange={e=>{setMyDate(e.target.value); loadMy(undefined, undefined, e.target.value);}}/></div>
                  <div className="col" style={{alignSelf:'end'}}><button className="secondary" onClick={()=>loadMy()}>تحديث</button></div></div>
                  <div className="list">
              ...
              {tab==='admin' && session.role==='admin' && (
                <div id="view-admin">
                  <div className="row"><div className="col"><label>تاريخ</label><input type="date" value={adDate} onChange={e=>{setAdDate(e.target.value); loadAdmin(e.target.value);}}/></div>
                  <div className="col" style={{alignSelf:'end'}}><button className="secondary" onClick={()=>loadAdmin()}>تحديث</button></div></div>
                  <div className="list">
